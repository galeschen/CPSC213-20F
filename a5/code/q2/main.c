...
int test_freelist1(struct testfunc *tf) {
  ...
  if(ptr != ptr2) {
    fprintf(stderr, "%s: mymalloc did not reuse free block %p (returned %p instead)\n",
      tf->name, ptr, ptr2);
    return -1;
  }
  return 0;
...
int test_freelist2(struct testfunc *tf) {
  ...
  /* Allocate two blocks of memory, then free them. */
  for(int i=0; i<2; i++) {
  ...
  /* Verify that a free-list based allocator reuses the two blocks above. */
  for(int i=0; i<2; i++) {
    void *ptr = mymalloc(FREELIST2_SIZE);
    ...
    if(ptr != p[0] && ptr != p[1]) {
      fprintf(stderr, "%s: mymalloc did not reuse either free block %p or %p (returned %p instead)\n",
        tf->name, p[0], p[1], ptr);
      return -1;
    }
    q[i] = ptr;
